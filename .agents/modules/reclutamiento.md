...
**Revisión de archivos sueltos (2026-09-23):** cuando el postulante subió un archivo por documento, el expediente tiene **Revisar con IA** (`POST /personal/reclutamiento/candidatos/:folderId/revisar-archivos`). No parte el PDF: califica cada casilla (¿el archivo de "Cédula" es una cédula?) con la misma alta/media/baja, y para cada archivo adicional — también si la postulación fue en un solo PDF — escribe una frase de qué documento parece ser. El resultado se guarda en `revision-archivos-ia.json` (no es un documento del postulante) y se vuelve a mostrar al abrir el expediente. PDF e imágenes; Word no se lee.
