...
export class VentasTemplatesService {
  ...
  async deleteTemplate(id: number, companyId: number | null) {
    ...
    const contractCount = await this.prisma.salesContract.count({
      where: { templateId: id },
    });
    if (contractCount > 0) {
  ...
  async downloadFromDrive(templateId: number, companyId: number | null) {
    ...
    try {
      ...
      const isGoogleDoc =
        template.driveUrl.includes('/document/') ||
        template.driveUrl.includes('/spreadsheets/') ||
        template.driveUrl.includes('/presentation/');
